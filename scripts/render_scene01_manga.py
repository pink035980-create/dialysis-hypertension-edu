...
# 1. Prepare 24.5s padded audio
audio_src = r"C:\AI\影片製作_透析高血壓衛教\audio\audio_scene01_synced.mp3"
audio_synced = os.path.join(output_dir, 'scene_01_manga_24s.mp3')
cmd_audio = ['ffmpeg', '-y', '-i', audio_src, '-af', 'apad=whole_dur=24.5', '-t', '24.5', audio_synced]
subprocess.run(cmd_audio, check=True)
print('24.5s padded audio prepared.')

# 2. Panels for Scene 1
...
def draw_frame(active_step, pulse):
    ...
    draw.text((60, 46), '【總覽開場】掌握透析控壓三大法寶：量血壓・管體重・遵醫囑！', font=f_header, fill=(255, 255, 255))
    ...
    if active_step != -1:
        ...
        rx = bx + bw + 24
        draw.text((rx, bot_y1 + 18), '① 血液透析病友心血管保護首重平穩血壓，居家定時量測 722 掌握真實數值！', font=f_bot_body, fill=(40, 50, 60))
        draw.text((rx, bot_y1 + 54), '② 乾體重與水分嚴格管理，兩次洗腎增幅小於 5%，飲食減鹽嚴禁高鉀低鈉鹽！', font=f_bot_body, fill=(185, 40, 25))
        ...
        rx = bx + bw + 24
        draw.text((rx, bot_y1 + 22), '「量血壓・管體重・遵醫囑」三大法寶！三分鐘帶您建立完整心腦保護力！', font=f_bot_title, fill=(192, 57, 43))
...
out_video = os.path.join(output_dir, '樣片_第1幕_片頭總覽_日式衛教漫畫連續動作版.mp4')
...
print('Rendering Scene 1 Japanese Manga Video...')
...
pipe.stdin.close()
pipe.wait()
print('Scene 1 Video rendered successfully:', out_video)
