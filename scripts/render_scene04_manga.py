...
# 1. Prepare 26.0s padded audio
audio_src = r"C:\AI\影片製作_透析高血壓衛教\audio\audio_scene04_synced.mp3"
audio_synced = os.path.join(output_dir, 'scene_04_manga_26s.mp3')
cmd_audio = ['ffmpeg', '-y', '-i', audio_src, '-af', 'apad=whole_dur=26.0', '-t', '26.0', audio_synced]
subprocess.run(cmd_audio, check=True)
print('26.0s padded audio prepared.')

# 2. Panels for Scene 4
p1_path = os.path.join(brain_dir, 's4_p1_calendar_zh.png')
p2_path = os.path.join(brain_dir, 's10_p1_bp_clean_1789452006932.jpg')
p3_path = os.path.join(brain_dir, 's4_p3_tworeadings_1789453179627.jpg')
...
def draw_frame(active_step, pulse):
    ...
    draw.text((60, 46), '【居家守則】落實居家血壓「７２２」原則：量七天・早晚量・量兩遍！', font=f_header, fill=(255, 255, 255))
    ...
    if active_step != -1:
        ...
        rx = bx + bw + 24
        draw.text((rx, bot_y1 + 18), '① 連續 7 天早晚各量 1 次，每次量 2 遍取平均值，能最有效去除外在偶然誤差！', font=f_bot_body, fill=(40, 50, 60))
        draw.text((rx, bot_y1 + 54), '② 居家目標血壓維持收縮壓 120～135、舒張壓 60～80 mmHg，每次洗腎請帶記錄本來院！', font=f_bot_body, fill=(185, 40, 25))
        ...
        rx = bx + bw + 24
        draw.text((rx, bot_y1 + 22), '「量7天、早晚量、量2遍、帶本子！」居家規律測量，掌握真實血管健康！', font=f_bot_title, fill=(192, 57, 43))
...
out_video = os.path.join(output_dir, '樣片_第4幕_居家血壓722原則_日式衛教漫畫連續動作版.mp4')
...
print('Rendering Scene 4 Japanese Manga Video...')
...
pipe.stdin.close()
pipe.wait()
print('Scene 4 Video rendered successfully:', out_video)
