...
# 1. Prepare 25.8s padded audio
audio_src = r"C:\AI\影片製作_透析高血壓衛教\audio\audio_scene06_synced.mp3"
audio_synced = os.path.join(output_dir, 'scene_06_manga_25s.mp3')
cmd_audio = ['ffmpeg', '-y', '-i', audio_src, '-af', 'apad=whole_dur=25.8', '-t', '25.8', audio_synced]
subprocess.run(cmd_audio, check=True)
print('25.8s padded audio prepared.')

# 2. Panels for Scene 6
p1_path = os.path.join(brain_dir, 's10_p2_weight_1789452027876.jpg')
p2_path = os.path.join(brain_dir, 'scale_water_bottles_zh.png')
p3_path = os.path.join(brain_dir, 'senior_measuring_scale_1789392716060.jpg')
...
def draw_frame(active_step, pulse):
    ...
    draw.text((60, 46), '【體重管理】兩次透析間體重增幅 ＜ ５％：60公斤病友最多增加 3 公斤！', font=f_header, fill=(255, 255, 255))
    ...
    if active_step != -1:
        ...
        rx = bx + bw + 24
        draw.text((rx, bot_y1 + 18), '① 乾體重是洗腎病友體內無多餘水分且無低血壓時的理想體重，兩次增幅嚴格控制 ＜ 5%！', font=f_bot_body, fill=(40, 50, 60))
        draw.text((rx, bot_y1 + 54), '② 每天固定時間量體重，若兩天內體重暴增超過 2 公斤，代表嚴重積水，請即刻節制飲水！', font=f_bot_body, fill=(185, 40, 25))
        ...
        rx = bx + bw + 24
        draw.text((rx, bot_y1 + 22), '「增幅不超百分之五、每天晨起量體重！」限水達標，透析無負擔、精神百倍！', font=f_bot_title, fill=(192, 57, 43))
...
out_video = os.path.join(output_dir, '樣片_第6幕_體重增幅不超5趴_日式衛教漫畫連續動作版.mp4')
...
print('Rendering Scene 6 Japanese Manga Video...')
...
pipe.stdin.close()
pipe.wait()
print('Scene 6 Video rendered successfully:', out_video)
