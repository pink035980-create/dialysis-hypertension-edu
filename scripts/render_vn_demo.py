...
f_step_tag = ImageFont.truetype(font_zh_path, 26)
f_card_sub = ImageFont.truetype(font_zh_path, 25)
f_card_body = ImageFont.truetype(font_zh_path, 23)
f_bot_title = ImageFont.truetype(font_zh_path, 26)
f_bot_body = ImageFont.truetype(font_zh_path, 23)
...
def render_frame_for_time(t):
    ...
    rx = bx + bw + 20
    draw.text((rx, bot_y1 + 12), '① 居家定時量測 722 掌握真實血壓！連續量 7 天、早晚各 2 遍取平均值', font=f_bot_body, fill=(40, 50, 60))
    draw.text((rx, bot_y1 + 44), '② 乾體重增幅嚴格控制小於 5%，飲食限鹽先限水，嚴禁食用高鉀低鈉鹽！', font=f_bot_body, fill=(185, 40, 25))
    draw.text((rx, bot_y1 + 76), '★ 成大醫院血液透析室諮詢專線：(06) 235-3535 分機 2591・專業團隊守護健康', font=f_bot_body, fill=(24, 76, 120))
    
    # 4. Bilingual Subtitle Banner at the very bottom (y: 962 ~ 1070)
    sub_bg_y1 = 962
    sub_bg_y2 = 1068
    draw.rounded_rectangle([140, sub_bg_y1, 1920 - 140, sub_bg_y2], radius=16, fill=(15, 23, 42, 240), outline=(56, 189, 248), width=2)
    
    # Clean Language Badge on Left (Arial Bold, 0 tofu boxes)
    tag_str = "TIẾNG VIỆT"
    ...
    # Render Bilingual Text (Geometrically Centered)
    if cur_zh:
        zh_w = draw.textlength(cur_zh, font=f_sub_zh)
        vn_w = draw.textlength(cur_vn, font=f_sub_vn)
        center_x = (1920 + 200) // 2
        draw.text((center_x - zh_w / 2, sub_bg_y1 + 14), cur_zh, font=f_sub_zh, fill=(255, 255, 255))
        draw.text((center_x - vn_w / 2, sub_bg_y1 + 54), cur_vn, font=f_sub_vn, fill=(254, 240, 138))
...
print("Rendering finished & copied to Desktop successfully!")
